...
- defineProperty 和 proxy 区别
    - 兼容性
      defineProperty ES5 中被引入，因此支持任何支持 ES5 版本的浏览器和环境  
...
- Modal 组件设计
    ...
    抽象
    - 组件设计
        点击新增与编辑都弹框出来进行填写，功能上大同小异，可能只是标题内容或者是显示的主体内容稍微不同
        props + slot 
    - 需求分析
        - 遮罩层
            Portal 挂在body上
        - 标题内容
        - 主体内容
            可以是字符串, 也可以是一段 html 代码
        - 确定和取消按钮
        除了通过引入import的形式，我们还可通过API的形式进行组件的调用
        配置全局样式、国际化、与typeScript
    - 实现流程
        - plugins
            - 目录结构
...
- SPA首屏加载速度慢的怎么解决
    ...
    - 解决方案
        - 减小入口文件体积
            路由懒加载  不同路由对应的组件分割成不同的代码块，待路由被请求的时候会单独打包路由，使得入口文件变小，加载速度大大增加
            以函数的形式加载路由
            ```
            routes:[ 
    path: 'Blogs',
    ...
    component: () => import('./components/ShowBlogs.vue')
        ...
        - 组件重复打包
            A.js文件是一个常用的库，现在有多个路由使用了A.js文件，这就造成了重复下载
            在webpack的config文件中，修改CommonsChunkPlugin
            minChunks: 3
            minChunks为3表示会把使用3次及以上的包抽离出来，放进公共依赖文件，避免了重复加载组件
        - 开启GZip压缩
        - 使用SSR
            在传统的客户端渲染模式中，页面的 HTML 和 JS 等资源是由客户端浏览器去请求，然后由浏览器进行下载、解析和渲染，最终呈现给用户。但客户端渲染的过程中，浏览器需要等待下载、解析和执行 JS 的过程，而这些过程往往都需要耗费不少时间，导致首屏加载时间较长。

            而在 SSR 模式中，服务端会直接将 HTML 页面生成并返回给浏览器，在浏览器接收到 HTML 页面后，就能够直接进行渲染，无需等待 JS 脚本下载、解析和执行。这样就能够大大缩短首屏加载时间，并且提升了 SEO（
            缺点
                - SSR 需要在服务器端对每个请求都进行 HTML 的构造，这会带来更大的服务器端负担和性能开销；
                - 在使用 SSR 的情况下，由于每次请求都需要重新构造 HTML，这对于需要维护大量动态内容或需要进行大量计算的网站来说，可能会导致加载时间过长；
                - 增加了项目的开发复杂度。
            - SPA 优点
                - SPA 的加载速度往往比 SSR 更快，因为服务器只需要向客户端提供一份基础 HTML 和 JS 包，页面首次渲染之后，其余的数据的请求都由客户端负责，不需要再次请求服务器
                - SPA 可以更好地支持复杂的交互式功能和动效，因为整个应用程序的渲染控制都交给了客户端，使得我们可以更容易地构建复杂的界面和交互体验。
                - SPA 的开发方式更加简单，无需使用额外的服务器端模板引擎和工具
...
- Composition API 可以说是Vue3的最大特点，那么为什么要推出Composition Api，解决了什么问题
    - options API
        methods  data   watch computed 生命周期  props 
        计划经济 
    - composition api
        组件根据逻辑功能来组织的，一个功能所定义的所有 API 会放在一起
        市场经济
        即使项目很大，功能很多，我们都能快速的定位到这个功能所用到的所有 API
    Vue2开发的项目  有以下问题
    1. 代码的可读性随着组件变大而变差
        当组件变得复杂时， 相关逻辑 要在 data  method 等地方蹦极
        this 的丢失  事件绑定....
        为了面向对象而面向对象
    2. TypeScript支持有限

    - Composition API几乎是函数，会有更好的类型推断  对ts 更友好
